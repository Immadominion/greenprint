...
- **Nature:** a demo or prototype, but it must genuinely work, with a UI good enough to impress the lecturer.
...
6. Detection of inefficient algorithms or coding patterns.
...
- 18 people in Group 3; all 18 are members and are included in the build. (An earlier note listed three names as the ones online at the time, so their reg numbers could be provided. None of them dropped, and all are part of the group.)
- Group leader: Ozuzu Victor Onyedikachi.
- Every member must have a distinct part to present and defend. The defense guide must be:
...
- The full class-list spreadsheet was used locally only to pull reg numbers; it is never published.
  Reg numbers are kept only in the team's private defense document (a local .docx), deliberately not in this public repo.
...
- Design references imported via the Claude Design MCP:
  - App: The Touchline and The Gaffer (project `f12a62f7-d717-45ed-8349-0b3f43cffa3f`).
  - Landing: Eddie (project `5bf85bcd-c916-44ea-afbf-0c88cd4b7a55`).
- Our own theme (chosen by us): clean white background, bright orange for action, emerald green for the eco and sustainability accents; runtime light and dark.
...
2. UI consistency. Every edit must match the existing design system and UI patterns.
3. Never present inferred, speculated, or deduced content as fact. Label unverified claims `[Inference]`, `[Speculation]`, or `[Unverified]`; say plainly when something cannot be verified. This is why every energy and CO2 number in the app is labelled a modelled estimate, never a measurement.
4. Proper documentation is non-negotiable for a final-year project: an in-app docs page (`/docs`), a How it works page (`/rules`), technical docs (`docs/`), and the separate plain-English defense guide.
5. Generate and maintain `.github/copilot-instructions.md` for AI coding agents on this repo.
6. Be strategic and self-directed: keep a roadmap ([`docs/ROADMAP.md`](ROADMAP.md)) and re-prompt for the next best step.
7. No em dashes in written material.
...
| 8 | Sustainability dashboard | EcoScore trend, CO2 saved, quality + green metrics: `src/app/(app)/dashboard/` |
...
| G2 | Algorithmic optimization | O(n^2) toward O(n) suggestions: `rules.ts` |
...
| - | Per-member defense guide | `docs/DEFENSE-GUIDE.md` (+ a formatted local .docx with reg numbers for the team) |
